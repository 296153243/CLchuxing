...
+ (NSArray *)getTwoMonthsDatesWithMonths:(NSInteger)months
{
    NSMutableArray *bigArray = [NSMutableArray array];
    NSMutableArray *dates = [NSMutableArray array];
    long long nowTime = [[NSDate date] timeIntervalSince1970], //开始时间
    endTime = [[[NSDate date] hyb_dateAfterMonth:months] timeIntervalSince1970],//结束时间
    dayTime = 24*60*60,
    ...
    while (time < endTime) {
        ...
        NSString *showOldDate = [formatter stringFromDate:date];
        
        FerryDateModel *model = [[FerryDateModel alloc]init];
        model.day = showOldDate;
        model.week = [date hyb_dayFromWeekday];
        
        if ([showOldDate hasSuffix:@"01"]) {
            //另一个月第一天
            [bigArray addObject:[NSArray arrayWithArray:dates]];
