...
+ (void)showMapNavSheetWithEndLat:(NSString *)lat lon:(NSString *)lon name:(NSString *)endName
{
    ...
    if ([[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:@"baidumap://"]]) {
        ...
        [alertView addButtonWithTitle:@"百度地图" type:HDAlertViewButtonTypeDefault handler:^(HDAlertView *alertView) {
            ...
            NSString *urlString = [[NSString stringWithFormat:@"baidumap://map/direction?origin={{我的位置}}&destination=%f,%f|name:%@&mode=walking&src=趣约车",ret.latitude,ret.longitude,endName] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    ...
    if ([[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:@"qqmap://"]]) {
        ...
        [alertView addButtonWithTitle:@"腾讯地图" type:HDAlertViewButtonTypeDefault handler:^(HDAlertView *alertView) {
            ...
            NSString *urlString = [[NSString stringWithFormat:@"qqmap://map/routeplan?type=walk&from=我的位置&tocoord=%@,%@&to=%@&referer=趣约车",lat,lon,endName] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
            NSURL *url = [NSURL URLWithString:urlString];
            [[UIApplication sharedApplication] openURL:url];
...
+ (void)trackALCustomHitBuildWithEventLabel:(NSString *)label controller:(UIViewController *)controller
{
    ...
    // 设置自定义事件标签
    [customBuilder setEventLabel:label];
    // 设置自定义事件页面名称
    [customBuilder setEventPage:NSStringFromClass([controller class])];
//    // 设置自定义事件持续时间
