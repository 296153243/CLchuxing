...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *headHightConstraint;
...
- (void)viewDidLoad {
    ...
    self.headHightConstraint.constant = SCREEN_NAV_HEIGHT;
    ...
    //初始化索引条
    self.indexView = [[MJNIndexView alloc]initWithFrame:CGRectMake(self.tableView.frame.origin.x, self.tableView.frame.origin.y, SCREEN_WIDTH, SCREEN_HEIGHT - SCREEN_NAV_HEIGHT)];
...
{
    if (![@"320500" isEqualToString:model.cityCode]) {
        [QuHudHelper qu_showMessage:@"该城市尚未开通！"];
        return;
    }
    //如果是切换的是相同城市，则无任何操作
                    ...
                    ([CLLocationManager authorizationStatus] == kCLAuthorizationStatusAuthorizedWhenInUse
                     || [CLLocationManager authorizationStatus] == kCLAuthorizationStatusNotDetermined || [CLLocationManager authorizationStatus] == kCLAuthorizationStatusAuthorizedAlways)) {
                        ...
                        [[QuLocationManager shareManager]startUpdatingLocationWithSuccess:^(AMapLocationReGeocode *lbsLocation) {
                            ...
                            [weakSelf.sectionArray replaceObjectAtIndex:0 withObject:[weakSelf addLocationCity]];
